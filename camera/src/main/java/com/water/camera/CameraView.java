...
import com.water.camera.view.ICameraView;
...
public class CameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
    ...
    /**
     * 是否可以切换摄像头,true可以并展示切换图标,false不可以并隐藏切换图标.
     */
    private boolean mSwitchCameraEnabled;
    /**
     * 设置是否可以切换闪光灯模式.
     */
    ...
    public CameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSwitchCameraEnabled = a.getBoolean(R.styleable.CameraView_switchCameraEnabled, true);
        mSwitchFlashEnabled = a.getBoolean(R.styleable.CameraView_switchFlashEnabled, true);
        leftButtonType = a.getInt(R.styleable.CameraView_leftButtonType, 0);
        previewEnabled = a.getBoolean(R.styleable.CameraView_previewEnabled, true);
        a.recycle();
        initData();
        initView();
    }
    ...
    private void initView() {
        ...
        mSwitchCamera.setVisibility(mSwitchCameraEnabled ? VISIBLE : GONE);
        mFlashLamp = view.findViewById(R.id.ivFlash);
        mFlashLamp.setVisibility(mSwitchFlashEnabled ? VISIBLE : GONE);
        setFlashRes();
        ...
        mCaptureLayout.setLeftButtonType(leftButtonType);
        ...
        mCaptureLayout.setCaptureListener(new CaptureListener() {
            ...
            public void takePictures() {
                ...
                machine.capture(previewEnabled);
            }
        ...
        switch (type) {
            case TYPE_VIDEO:
                ...
                if (mOnCameraListener != null) {
                    mOnCameraListener.recordSuccess(videoUrl, firstFrame);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
                if (mOnCameraListener != null) {
                    mOnCameraListener.captureSuccess(captureBitmap);
                }
                break;
            case TYPE_SHORT:
            case TYPE_DEFAULT:
