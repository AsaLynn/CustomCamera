...
import static com.water.camera.CameraView.BUTTON_STATE_ONLY_RECORDER;
...
public class CaptureButton extends View {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mCaptureListener != null
                    ...
                    mCaptureListener.recordZoom(event_Y - event.getY());
                }
                break;
            case MotionEvent.ACTION_UP:
                //根据当前按钮的状态进行相应的处理
                handlerUpPressByState();
                break;
        }
        return true;
    }
    ...
    //当手指松开按钮时候处理的逻辑
    private void handlerUpPressByState() {
        removeCallbacks(longPressRunnable); //移除长按逻辑的Runnable
        //根据当前状态处理
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (mCaptureListener != null && (button_state == BUTTON_STATE_ONLY_CAPTURE || button_state ==
                        BUTTON_STATE_BOTH)) {
    ...
    private void recordEnd() {
        if (mCaptureListener != null) {
            if (recorded_time < min_duration)
                ...
                mCaptureListener.recordEnd(recorded_time);  //回调录制结束
        }
    ...
    //设置回调接口
    public void setCaptureLisenter(CaptureListener mCaptureListener) {
